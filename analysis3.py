...
focus = output[
    (output["需求量"] > 0) |
    (output["既有在途到貨量"] > 0) |
    (output["MRP採購到貨量(今日)"] > 0) |
    (output["當日建議下單量"] > 0) |
    (output["低於安全庫存"])
].copy()

print("\n=== 未來 7 天零件供需模擬（MRP升級版｜重點） ===")
print(focus.sort_values(["低於安全庫存", "日期"], ascending=[False, True]).head(120))
...
po_summary = (
    output[output["當日建議下單量"] > 0]
    .groupby("零件圖號")
    .agg(
        七天建議下單總量=("當日建議下單量", "sum"),
        最早下單日=("日期", "min"),
        最早到貨日=("本日下單預計到貨日", "min"),
    )
    .reset_index()
    .sort_values("七天建議下單總量", ascending=False)
)

print("\n=== 7天採購建議彙總（更像PO清單） ===")
print(po_summary.head(50))
